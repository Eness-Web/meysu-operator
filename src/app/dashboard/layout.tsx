...
export default function DashboardLayout({ children }: { children: React.ReactNode }) {
  ...
  useEffect(() => {
    ...
    supabase.auth.getSession().then(({ data: { session } }) => {
      ...
      if (adminRoles.includes(role)) { router.replace("/admin/dashboard"); return; }
      ...
      <header className="bg-[#1a237e] text-white px-6 py-3 flex items-center justify-between shadow-lg">
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-xs text-blue-300">Makine Takip Sistemi</p>
          </div>
        </div>
        <div className="flex items-center gap-3">
          <div className="text-right">
            <p className="text-sm font-bold">{session.displayName}</p>
            <p className="text-xs text-blue-300">{session.role}</p>
          </div>
          ...
          return (
            <Link key={item.href} href={item.href}
              className={cn(
                ...
                active ? "bg-[#1a237e] text-white shadow-inner" : "bg-[#3949ab] text-white hover:bg-[#1a237e]"
              )}>
